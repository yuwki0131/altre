...
> **更新 (2025-10-20):** T1/T2/F1/F2 の作業は完了し、GUI からのキー入力・ファイル操作は一往復で反映できる状態になった。未着手タスクは T3（スナップショット拡張）、F3/F4（UI/Fallback 調整）、QA1（E2E 手順検討）を中心に継続する。

## 1. 背景
Tauri GUI プロトタイプは `invoke` コマンドと React フロントエンドを用意済みだが、以下の理由で完全な往復フロー（入力→バックエンド→スナップショット→描画）が未完成である。

- `altre-tauri` のコマンド群は Pull 型スナップショットを返すのみで、初期化やエラー挙動の仕様が明文化されていない。（※2025-10 時点で `editor_init` / `editor_save_file` 等を実装し、仕様をコード化済み）
- React 側の `services/backend.ts` は Tauri ランタイム未接続時のフォールバックに重点を置いており、実機接続時のエラーハンドリング・更新タイミングが暫定的。（※`useEditor` でエラー/情報メッセージ統合済み）
- キーシーケンスの正規化が 1 ストローク単位に限定されており、`C-x C-f` など複合キーを扱う拡張が必要。（※`KeySequencePayload` のチャンク化と複合キー送信を実装済み）
