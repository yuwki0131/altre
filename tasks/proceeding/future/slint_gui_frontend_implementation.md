...
## 進捗メモ（2025-09-28）
- `src/frontend/gui/` 以下に Slint コンポーネントと `GuiApplication` を追加し、バックエンドからのデータ反映とキー入力の橋渡しを実装。
- Slint 依存は `gui` フィーチャーで有効化。ネットワーク制限のためクレート取得・GUI ビルド確認は未実施（QA Q27）。
- TUI モード (`cargo check`) は回帰なし。GUI 動作確認は依存解決後に実施する予定。
